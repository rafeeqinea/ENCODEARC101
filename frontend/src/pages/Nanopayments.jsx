...
import { Banknote, ArrowRight, Clock, Globe, ShieldCheck, Zap, Layers, ExternalLink } from 'lucide-react'
                ...
                <div className="flex flex-col md:flex-row items-stretch gap-2">
                    {CPN_FLOW.map((step, i) => (
                        <div key={step.step} className="flex-1 flex items-center gap-2">
                            <div className={`flex-1 p-3 rounded-xl border ${step.status === 'active' ? 'border-[var(--color-success)]/40 bg-[var(--color-success)]/5' : 'border-[var(--color-border-light)] bg-[var(--color-bg-secondary)]'}`}>
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className={`text-[0.5rem] font-bold uppercase px-1 py-0.5 rounded ${step.status === 'active' ? 'bg-[var(--color-success)]/20 text-[var(--color-success)]' : 'bg-[var(--color-text-muted)]/20 text-[var(--color-text-muted)]'}`}>
                                        {step.status}
                                    </span>
                                </div>
